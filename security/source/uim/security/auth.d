...
// Safe conversions are explicitly marked @trusted
...
string generateCsrfToken(string secret, string sessionId, SysTime now = Clock.currTime()) @trusted {
  ...
  return format("%s.%s", timestamp, cast(string) Base64URLNoPadding.encode(sig));
}

/// Verifies a CSRF token generated by `generateCsrfToken` within an optional max age.
bool verifyCsrfToken(string token, string secret, string sessionId, long maxAgeSeconds = 7200, SysTime now = Clock.currTime()) @trusted {
