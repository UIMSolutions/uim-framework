module uim.services.infrastructure.persistence.tenants.tenant;
...
class StoreTenant {
    ...
    /// Register store
    void register(string name, IStore store) {
        _stores[name] = store;
    }
    ...
    Store!(A, B) store(A, B)(string name) {
        return (name in _stores) ? cast(Store!(A, B))_stores[name] : null;
    }
}
