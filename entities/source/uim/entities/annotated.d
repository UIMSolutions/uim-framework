 ...
 */
string[] validateAnnotatedEntity(T)(T entity) 
        ...
        static if (hasUDA!(__traits(getMember, T, fieldName), Range)) {
          {
            ...
            static if (is(typeof(value) : long)) {
              if (value < attr.min || value > attr.max) {
                errors ~= fieldName ~ " must be between " ~ attr.min.to!string ~ " and " ~ attr.max.to!string;
              }
            }
          }
        }
 ...
 */
string[] getAnnotatedFieldNames(T)() 
    if (is(T == class) || is(T == struct)) {
 ...
 */
string[string] toAnnotatedMap(T)(T entity) 
    if (is(T == class) || is(T == struct)) {
 ...
 */
void fromAnnotatedMap(T)(T entity, string[string] data) 
    if (is(T == class)) {
  import std.traits : hasUDA, getUDAs, isCallable;
...
unittest {
  ...
  newProduct.fromAA(["name": "Mouse", "price": "29.99", "sku": "SHOULD-NOT-SET"]);
